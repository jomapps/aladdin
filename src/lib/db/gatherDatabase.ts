...
class GatherDatabaseManager {
   ...
   */
  async createGatherItem(projectId: string, item: Omit<GatherItem, '_id' | 'createdAt' | 'lastUpdated'>): Promise<GatherItem> {
   ...
   */
  async getGatherItems(projectId: string, options: GatherQueryOptions = {}): Promise<GatherQueryResult> {
    const collection = await this.getGatherCollection(projectId)
